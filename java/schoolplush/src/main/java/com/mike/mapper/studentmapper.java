...
import com.mike.mapper.Provider.studentsearch;
...
public interface studentmapper {
    ...
    @SelectProvider(type = studentsearch.class, method = "search")
    List<student> search(Search search);

    @SelectProvider(type = studentsearch.class, method = "gettotal")
    int GetTotal(Search search);

    // 保存头像
    @Update("update student set file=#{file} where id=#{id}")
    int savefile(student a);

    // 获取头像
    @Select("select file from student where id=#{id}")
